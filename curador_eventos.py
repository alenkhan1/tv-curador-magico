...
# pero hora_corta se usa internamente en el scoring de streams con hora en su nombre.
...
def obtener_eventos_api() -> list:
    ...
    for deporte, sport_id in DEPORTES_IDS.items():
        ...
        r_cat = hacer_peticion_rotativa(
            f"https://{RAPIDAPI_HOST}/v1/calendar/categories",
            {"sport_id": sport_id, "date": FECHA_HOY, "timezone": 0}
        )
        if not r_cat or r_cat.status_code != 200:
            log.warning(f"  [{deporte}] Sin categorías.")
            continue

        try:
            categorias_activas = r_cat.json().get("data", [])
        except ValueError:
            log.error(f"  [{deporte}] JSON inválido en categorías.")
            continue
        ...
        for cat in categorias_activas:
            ...
            if deporte == "Fútbol" and cat_nombre not in CATEGORIAS_FUTBOL_PERMITIDAS:
                continue

            r_ev = hacer_peticion_rotativa(
                f"https://{RAPIDAPI_HOST}/v1/events/schedule/category",
                {"category_id": cat_id, "date": FECHA_HOY}
            )
            ...
            for ev in datos:
                try:
                    ...
                    # Filtros por deporte
                    if deporte == "Tenis" and "ITF" in torneo_norm and tier == 3:
                        continue
                    ...
                    log.warning(
                        f"  [{deporte}/{cat_nombre}] Evento {ev.get('id','?')}: "
                        f"{type(e).__name__}: {e}"
                    )
                    continue
